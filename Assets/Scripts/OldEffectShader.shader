Shader "Hidden/OldEffectShader"
 {
	Properties 
	{
		...
		_EffectAmount ("Old Effect Amount", Range(0,1)) = 1.0
		_StainAmount ("Stain Opacity", Range(0,1)) = 1.0
	...
	SubShader 
	{
		Pass
		{
			...
			fixed4 frag(v2f_img i) : COLOR
			{
				...
				//Get the pixels from the Stain Texture
				fixed4 vignetteTex = tex2D(_StainTex, i.uv);
				...
				finalColor = lerp(finalColor, finalColor * vignetteTex, _StainAmount);
